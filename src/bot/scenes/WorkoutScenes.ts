...
export function createWorkoutScenes(trainingManager: TrainingManager) {
    ...
    addExerciseScene.on("text", async (ctx) => {
        ...
        await ctx.reply(`YEAH BUDDY! Starting ${exerciseName}! 🏋️‍♂️\n` + "Use the buttons below to record your sets or add more exercises!", keyboard);
    ...
    addSetScene.on("text", async (ctx) => {
        ...
        if (!exercise) {
            const keyboard = Markup.inlineKeyboard([[Markup.button.callback("Add Exercise 🎯", "addExercise")]]);
